...
### Argument-first revision foundation (Phases 1–7)

A foundation layer for academic-grade revision. Each phase ships a working
slice; together they replace scattered single-purpose panels with one
coherent revision surface. Plan: [`docs/IMPLEMENTATION_PLAN.md`](docs/IMPLEMENTATION_PLAN.md).

- **Phase 1 — Product vocabulary cleanup.** All UI strings now use the activity model (Ingest · Scaffold · Draft · Find gaps · Refine · Restructure · Review). The retired quick/standard/deep "review levels" no longer appear in any user-facing surface; `runner.py` is preserved as legacy. `project_state()` returns a `next_activity` field so every screen can show "what next" from one source of truth.
- **Phase 2 — Graph visualisation correctness.** Voice-aware drafts directory resolution (the visualiser now reads `.lattice/drafts/<voice>/cluster_*.md` instead of the flat `.lattice/drafts/`). [Cytoscape vendored locally](src/lattice/web/static/vendor/cytoscape/) so the web UI works offline. The edge inspector surfaces relationship `note` and `strength` as the headline content.
- **Phase 3 — Revision Cockpit.** Four-pane working surface (paper preview · argument map · issues queue · claim/evidence inspector) replacing the audit/voice/gap/restructure/review subtabs as the default Output view. Single selection model: clicking a queue item, paragraph, or graph node syncs all four panes. New endpoints: `cockpit-queue`, `cockpit-claim/{id}`, `cockpit/actions/{action}`.
- **Phase 4 — Evidence retrieval and trace model.** [BM25-ranked binder](src/lattice/enricher/binder.py) replaces the head-of-document scan that silently dropped relevant passages past index 40. `Evidence` rows now carry `passage_char_start`/`_end`/`confidence` (optional, back-compat). New [`renderer/trace.py`](src/lattice/renderer/trace.py) emits `.lattice/paragraph_traces.<voice>.json` mapping every paragraph → sentence → claim_ids → source_ids → evidence_spans; the coverage check consumes this trace when present.
- **Phase 5 — Mechanism discipline + structured proposals.** `classify_mechanism_support` labels each claim's mechanism as `source_supported` / `author_specified` / `unknown` / `none`; the renderer prompt now forbids smoothing unsupported mechanisms into prose and instructs Claude to emit `{UNRENDERABLE_MECHANISM: ...}` markers (audited as critical via `coverage.unrenderable_mechanism_marker`). Five [`RedraftMode`](src/lattice/graph/models.py) variants. New [`ReviewProposal`](src/lattice/review/review.py) model carrying `affects_claim_ids`/`affects_source_ids`/`affects_relationship_ids`; cockpit accept/reject actions persist to `.lattice/proposal_decisions.<voice>.json`. `verify_claims_preserved` enforces "no claim may silently disappear in a rewrite."
- **Phase 6 — Map modes.** Five [cytoscape overlay modes](src/lattice/output/visualise.py): `thesis_support_path`, `section_proof_chain`, `weak_evidence_zones`, `counterargument_map`, `unrenderable_clusters`. Sidebar mode picker plus a cockpit-pane mode picker; bidirectional `postMessage` selection sync between the cockpit and the iframe (`lattice:set-mode`, `lattice:select-claim`, `lattice:node-tapped`).
- **Phase 7 — Provenance + versioning.** [`Snapshot`](src/lattice/graph/models.py) bundles graph + clusters + sources + audit flags into a single restorable JSON file at `.lattice/snapshots/<id>.json`. The activity dispatcher takes a snapshot before every non-ingest activity (`SnapshotKind.before_<verb>`); reverts always take a `pre_revert` snapshot first. New [`differ/graph_diff.py`](src/lattice/differ/graph_diff.py) reports adds/removes/modifies for every entity type. Cockpit "History" sub-view lists snapshots and offers Diff vs current + Revert. Phase 5's `verify_claims_preserved` is now wired to redraft activities: a rewrite that drops claims marks the cluster `failed`, blocks delivery, and is recoverable via the pre-activity snapshot.
