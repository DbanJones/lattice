...
def create_app(projects_root: Path | None = None) -> FastAPI:
    ...
    async def get_graph_viz(name: str) -> FileResponse:
        ...
        cytoscape-config fixes propagate without requiring users to
        ...
        candidate_inputs = [
            ...
            lattice_dir / "source_store.json",
            lattice_dir / "scaffold_report.json",
        ]
        # Pick up any audit/readiness output (filenames vary per voice).
        audit_dir = lattice_dir / "audit"
        if audit_dir.exists():
            try:
        ...
        if not needs_regen:
            try:
                viz_mtime = viz_path.stat().st_mtime
            except OSError:
                viz_mtime = 0.0
            for input_path in candidate_inputs:
                if not input_path.exists():
                    continue
                try:
                    if input_path.stat().st_mtime > viz_mtime:
                        needs_regen = True
                        break
                except OSError:
                    continue
        ...
        if needs_regen:
            ...
            # Best-effort: thread audit + readiness into the diagram so
            # cluster compound nodes get badges. Failures are non-fatal —
            # we'd rather render a slightly less-rich diagram than 500.
            audit_flags_by_cluster: dict[str, int] = {}
                ...
                if audit_dir.exists():
                    for p in audit_dir.iterdir():
                        ...
                        try:
                            ...
                            continue
                        # Audit flag files: a list of flags with cluster_id.
                        if isinstance(payload, list):
                            for flag in payload:
                                cid = flag.get("cluster_id") if isinstance(flag, dict) else None
                                if cid:
                                    audit_flags_by_cluster[cid] = (
                                        audit_flags_by_cluster.get(cid, 0) + 1
                                    )
                        # Readiness file: dict with blocking_clusters list.
                        elif isinstance(payload, dict):
                            for cid in payload.get("blocking_clusters", []) or []:
                                blocking.add(cid)
            except OSError:
                pass
